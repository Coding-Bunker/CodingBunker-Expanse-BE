...
open class Logging(config: Configuration) {
    ...
    private val log: Logger = config.logger ?: LoggerFactory.getLogger(Logging::class.java)
        ...
        log.info(StringBuilder().apply {
            ...
            if (logBody) {
                try {
                    ...
                    log.error(
                        "Logging payloads requires DoubleReceive feature to be installed with receiveEntireContent=true",
                        e
                    )
                }
            }
    ...
    protected open fun logResponse(call: ApplicationCall, subject: Any) {
        log.info(StringBuilder().apply {
            ...
            if (logHeaders) {
                call.response.headers.allValues().toMap().forEach { (header, values) ->
                    appendLine("$header: ${values.firstOrNull()}")
                }
            }
